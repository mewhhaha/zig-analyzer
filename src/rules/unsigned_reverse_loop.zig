...
pub fn run(context: RuleRun) !void {
    ...
    for (context.tokens, 0..) |token, declaration_index| {
        if (token.tag != .keyword_var or declaration_index + 4 >= context.tokens.len or
            ...
            context.tokens[declaration_index + 3].tag != .identifier or
            !isUnsignedType(context.tokenText(declaration_index + 3)) or
            context.tokens[declaration_index + 4].tag != .equal) continue;
