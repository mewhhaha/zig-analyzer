...
fn builtinSummary(tag: std.zig.BuiltinFn.Tag) []const u8 {
    return switch (tag) {
        .import => "Imports a Zig or ZON module and returns its namespace type or interpreted value.",
        .embed_file => "Embeds a file's bytes into the compilation as a pointer to a constant byte array.",
        .as => "Performs a type coercion using an explicitly supplied result type.",
        .int_cast => "Converts between integer types while preserving the numerical value; an out-of-range result is safety-checked illegal behavior.",
        .float_cast => "Converts between floating-point types using the inferred result type.",
        .int_from_float => "Converts a floating-point value to an integer by removing its fractional part.",
        .float_from_int => "Converts an integer value to a floating-point value.",
        .bit_cast => "Reinterprets a value's bit pattern as another type with the same bit size.",
        ...
        .enum_from_int => "Converts an integer tag value to an enum value of the inferred result type.",
        ...
        .TypeOf => "Returns the resolved peer type of one or more expressions.",
        .type_info => "Returns a compile-time description of a type.",
        .type_name => "Returns the fully qualified name of a type.",
        .Int, .Enum, .Union, .Struct, .Pointer, .Fn, .Vector, .Tuple => "Constructs a type from compile-time type information.",
        .size_of => "Returns the byte size of a type.",
        .bit_size_of => "Returns the number of bits required to represent a type.",
        .align_of => "Returns a type's ABI alignment.",
        .offset_of => "Returns the byte offset of a field within a container type.",
        .bit_offset_of => "Returns the bit offset of a field within a container type.",
        .field => "Accesses a field whose name is known at compile time.",
        ...
        .This => "Returns the innermost struct, enum, union, or opaque type containing the call.",
        ...
        .memcpy => "Copies bytes between non-overlapping memory regions.",
        .memmove => "Copies bytes between memory regions that may overlap.",
        .memset => "Sets every element in a memory region to the same value.",
